...
fn main() {
    let unparsed_file = fs::read_to_string("examples/data.json").expect("cannot read file");

    let json: JSONValue = parse_json_file(&unparsed_file).expect("unsucessful parse");
